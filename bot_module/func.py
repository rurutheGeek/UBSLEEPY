...
def make_json(filename: str,dict:dict):
    """jsonファイルを作成する
    Parameters:
    ----------
    filename : str
    ...
    if not dict is None:
        try:
            with open(f"save/{filename}.json", mode="x") as new_json:
                json.dump(dict, new_json, indent=4)
                output_log(f"{filename}.jsonを新規作成しました")
                return True
        except FileExistsError:
            output_log(f"document/default{filename}.jsonがすでに存在します")
            return True
    else:
        try:
            if not os.path.exists(f"save/{filename}.json"):
                try:
                    with open(f"document/default{filename}.json", mode="r") as default_json:
                        data = json.load(default_json)
                        with open(f"save/{filename}.json", mode="w") as new_json:
                            json.dump(data, new_json, indent=4)
                            output_log(f"default{filename}.jsonから{filename}.jsonを新規作成しました")
                            return True
                except FileNotFoundError:
                    output_log(f"document/default{filename}.jsonが見つかりませんでした")
                    return False
        except FileExistsError:
            return True
    

# jsonファイルを読み込む(キャッシュを使用)
@lru_cache
def get_json(filename: str, *args):
    if make_json(filename):
        output_log(f"save/{filename}.jsonが見つかりませんでした")
        return None
    try:
        with open(f"save/{filename}.json", "r") as file:
            data = json.load(file)
        for arg in args:
            data = data[arg]
        return data
    except KeyError:
        output_log(f"save/{filename}.jsonにキー[{arg}]が見つかりませんでした")
        return None


# jsonファイルを更新する(キャッシュをクリア)
def update_json(filename: str,value, *args):
    if make_json(filename):
        output_log(f"save/{filename}.jsonが見つかりませんでした")
        return None
    try:
        with open(f"save/{filename}.json", "w") as file:
            data = json.load(file)
        # argsをキーとしてvalueを代入し,jsonファイルを更新する
        for arg in args[:-1]:
            data = data[arg]
        data[args[-1]] = value
...
def fetch_pokemon(input: str) -> pd.DataFrame:
    ...
    # データベースをカタカナに
    kata_breloom_df = GLOBAL_BRELOOM_DF.iloc[:, 1:5].applymap(
        lambda x: jaconv.hira2kata(str(x))
    )
    SearchedData = kata_breloom_df[
        (kata_breloom_df["おなまえ"] == fixedName)
        | (kata_breloom_df["インデックス1"] == fixedName)
        | (kata_breloom_df["インデックス2"] == fixedName)
        | (kata_breloom_df["インデックス3"] == fixedName)
    ]
...
def filter_dataframe(filter_dict):
    ...
    for key, value in filter_dict.items():
        if key == "タイプ":
            filteredPokeData = filteredPokeData[
                (filteredPokeData["タイプ1"].isin(value))
                | (filteredPokeData["タイプ2"].isin(value))
            ]
        elif key == "特性":
            filteredPokeData = filteredPokeData[
                (filteredPokeData["特性1"].isin(value))
                | (filteredPokeData["特性2"].isin(value))
                | (filteredPokeData["隠れ特性"].isin(value))
            ]
        elif value[0].isdecimal():
            filteredPokeData = filteredPokeData[
                filteredPokeData[key].isin([int(v) for v in value])
            ]
        else:
...
def bss_to_text(values) -> str:
    ...
    if isinstance(values, list):
        ...
        bss = [
            int(values.iloc[0]["HP"]),
            int(values.iloc[0]["こうげき"]),
            int(values.iloc[0]["ぼうぎょ"]),
            int(values.iloc[0]["とくこう"]),
            int(values.iloc[0]["とくぼう"]),
            int(values.iloc[0]["すばやさ"]),
        ]
...
def generate_graph(bss: list[int]) -> str:
    ...
    graph_path=get_json("path","graph")
    ...
    ax.plot(
        angles, radar_values, color="midnightblue", alpha=0.4, linewidth=0.5
    )  # 　レーダーチャートの内側を塗りつぶす
    ax.fill(angles, radar_values, alpha=0.9, color="midnightblue")  # 項目ラベルの表示
    ax.set_thetagrids(
        angles[:-1] * 180 / np.pi, labels, fontweight="roman"
    )  # 円形の目盛線を消す
    ...
    for grid_value in rgrids:
        ...
        ax.plot(
            angles, grid_values, color="gray", linewidth=0.5, alpha=0.3
        )  # メモリの値を表示する
    ...
    output_log(f"種族値グラフ生成完了: {graph_path}")
    return graph_path
...
def report(userId, repoIndex: str, modifi: int) -> int:
    ...
    レポート後の値
    """
    report_path=get_json("path","report")
    reports = pd.read_csv(report_path, index_col=0)

    if repoIndex not in reports.columns:
        reports[repoIndex] = 0
        reports.to_csv(
            report_path, index=True, index_label="ユーザーID", float_format="%.0f"
        )
        ...
        if values[i] in GLOBAL_BRELOOM_DF["進化段階"].unique().tolist():
            ...
            for key in BASE_STATS_DICT.keys():
                if values[i].upper().startswith(key):
                    ...
                    break
        elif values[i] in np.unique(
            GLOBAL_BRELOOM_DF[["特性1", "特性2", "隠れ特性"]].astype(str).values.ravel()
        ):
            dictIndex = "特性"
        else:
            continue
...
def show_calendar(
    day: datetime = datetime.now(ZoneInfo("Asia/Tokyo")),
) -> discord.Embed:
    calendarTitle = BALL_ICON
    pokecalendar_path=get_json("path","pokecalendar")
    if day.date() == datetime.now(ZoneInfo("Asia/Tokyo")).date():
        calendarTitle += (
            f'{day.strftime("%Y/%m/%d")} ({WEAK_DICT[day.weekday()]}) 今日のできごと'
        )
    else:
    ...
    matched_rows = history_df[
        history_df["日付"].dt.strftime("%m/%d") == day.strftime("%m/%d")
    ].fillna("")
    ...
    createdEmbed = discord.Embed(
        title=calendarTitle, color=0x7ED321, description=calendarDescription
    )
...
def show_senryu(unique: bool = False) -> discord.Embed:
    pokesenryu_path=get_json("path","pokesenryu")
