...
from DB_access_sqlite import DBCommon, PostTable
from server import keep_alive #botの連続稼働用
...
RESOURCES_LINK = "https://pokecries.nobody.jp/content/resources/"
#参照データ
GROBAL_BRELOOM_DF = pd.read_csv('resources/pokedata_breloom.csv')
...
BOLL_ICON='<:bullet:1077833761313525820> '
...
BAKUSOKU_MODE=True
QUIZNAME_DICT={"種族値クイズ": "bq","物理特殊クイズ": "acq","英和翻訳クイズ": "etojq","和英翻訳クイズ": "jtoeq","中日翻訳クイズ": "ctojq"} #クイズ名を識別子に変換する辞書(スラッシュコマンドも更新される)
...
> **フィードバック機能**
・`/wish ご意見など`
半角スペース区切りで不具合報告・ご意見・ご感想・ご要望などを投稿
botの開発者たちが確認できる場所に共有される

> **ポケモンずかん機能**
・`/dex ポケモン名`
指定ポケモンの図鑑データを表示する
ポケモン名指定のバリエーションも確認できる

> **ポケモン検索機能**
・`/search 条件指定`
半角スペース区切りで条件を並べて合致するポケモンを表示する
条件の指定方法は以下の通り
```md
#タイプ
ポケモン18タイプの正式名称
#出身地
ポケモン本編の地方
['カントー','ジョウト','ホウエン','シンオウ','イッシュ','カロス','アローラ','ガラル','ヒスイ','パルデア']
#初登場世代
ポケモン作品の世代区分(ピカブイは7,アルセウスは8扱い)
['1','2','3','4','5','6','7','8','9']
#進化段階
ポケモンの進化段階
['進化前','中間進化','最終進化','進化しない']
...
ポケモンの種族値
H108 や A130 や S102 のように各ステータスを表す英字と数字をつなげて指定
```
> **種族値クイズ機能**
・`/q` または <@!1076387439410675773>(botにメンション)
現在の出題設定に基づいてクイズを出題する
初期出題設定は{'進化段階':['最終進化','進化しない']}
・`ポケモン名` (クイズ投稿に返信) 
正誤判定を行う
ポケモン名はある程度の表記ゆれに対応している
・`['ヒント','特性','地方','分類', '作品'] のうちどれか`(クイズ投稿に返信)
それぞれについてヒントを表示する(ひらがなやカタカナでも可)
ヒントをリプライした場合は未表示のヒントからランダムで表示
・`["ギブ","ギブアップ","降参","敗北"]のうちどれか`(クイズ投稿に返信) 
種族値クイズの正解を表示する
・`/bmode` または <:botan:1082971898293522482>(絵文字単体を送信)
正解が表示されると次のクイズを出題する、連続出題モードのオンオフを切り替える
初期設定はオフ
・`/bqdata 出題条件`
半角スペース区切りで出題条件を並べて出題設定を変更できる
出題条件の指定方法はポケモン検索機能`/search`と同じ
タイプ・出身地といったカテゴリー名を出題条件として指定するとそのカテゴリーの条件を削除する
リセット を出題条件として指定すると初期設定に戻る
出題条件を設定せずに送信で現在の出題設定を確認する
・`/quizrate`
送信したユーザーの種族値クイズの正答率が表示される
・`/graph ポケモン名`
指定ポケモンの種族値クイズで使用されるグラフを表示する

> **実数値計算機能**
・`/stats 各数値`
各数値からポケモンの実数値を計算する
初期値が設定されている項目は変更の必要がなければ記入不要

> **招待機能**
・`/invite メンバー名 匿名性`
サーバーのメンバーを使用チャンネルに招待できる
招待者を秘匿するか選択可能 初期値は公開
...
> **/johto**
・`/johto メンバー名`
自分がステージチャンネルのホストのとき、ホストを他のメンバーに譲渡する

> **あいことば生成機能**
・`/pass`
8ケタの数字をランダム生成する
通信時にあいことばを決めるのが面倒な際につかえるかも

> **ポケモン川柳機能**
/senryu
オーキドのポケモン川柳からランダムでひとつ表示する

> **今日はなんの日機能**
/calendar
指定した月日がポケモンの記念日であるか表示する
月日の初期値は今日になっている
'''
  ...
  helpEmbed.set_author(name='BOT せつめいしょ',
    icon_url=f"{RESOURCES_LINK}icon/Breloom.png"
  )
...
> **シャットダウン機能**
・`/spore`
botの一部記憶を保持したままシャットダウンすることができる

> **ステータスメッセージ変更機能**
・`/notice ステータスメッセージ`
xxをプレイ中といったbotのステータス表記を変更できる
    '''
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(quizname='クイズの種別 未記入で種族値クイズが指定されます')
shunsanhate='''@discord.app_commands.choices(quizname=[discord.app_commands.Choice(name=val, value=val) for val in list(QUIZNAME_DICT.keys())])
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(user='表示したいメンバー名',quizname='クイズの種別 未記入で種族値クイズが指定されます')
...
async def slash_quizrate(interaction: discord.Interaction, user: discord.Member=None,quizname: str="種族値クイズ"):
  ...
  w=report(showId, f'{QUIZNAME_DICT[quizname]}正答', 0)
  l=report(showId, f'{QUIZNAME_DICT[quizname]}誤答', 0)
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(mode='連続出題モードのオンオフ 未記入でトグル切り替え')
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(name='表示したいポケモンのおなまえ')
async def slash_graph(interaction: discord.Interaction, name: str):
  graphDatas = fetch_pokemon(name)
  ...
  if graphDatas is not None:    
    ...
    graphEmbed = discord.Embed(
      ...
      description=bss_to_text(graphDatas)
    )
    file = discord.File(generate_graph(baseStats), filename="image.png")
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(h="HP種族値",a="こうげき種族値",b="ぼうぎょ種族値",c="とくこう種族値",d="とくぼう種族値",s="すばやさ種族値")
async def slash_graphbss(
  interaction: discord.Interaction,
  h: app_commands.Range[int, 1,255], a:  app_commands.Range[int, 1,255],
  b:  app_commands.Range[int, 1,255], c:  app_commands.Range[int, 1,255],
  d:  app_commands.Range[int, 1,255], s:  app_commands.Range[int, 1,255]
):
  baseStats = [h,a,b,c,d,s]
  await interaction.response.defer() #レスポンス遅延のおまじない
  graphEmbed = discord.Embed(
    title="種族値グラフ",
    description=bss_to_text(baseStats)
  )
  file = discord.File(generate_graph(baseStats), filename="image.png")
  graphEmbed.set_image(url="attachment://image.png")
  await interaction.followup.send(file=file,embed=graphEmbed)
...
async def slash_dex(interaction: discord.Interaction, name: str):
  ...
  if (pokedata:= fetch_pokemon(name))is not None: #データが存在する場合は、図鑑データを返信
    ...
    dexEmbed = discord.Embed(
      title=f'{BOLL_ICON}{dexName}の図鑑データ{BOLL_ICON}',
...
```
    ...
    dexEmbed.set_thumbnail(url=f'{RESOURCES_LINK}art/{dexNumber}.png')
    ...
    if (GROBAL_BRELOOM_DF["ぜんこくずかんナンバー"] == int(float(dexNumber))-1).any():
      dexEmbed.add_field(
        name=f'No.{int(float(dexNumber))-1}',
        value=f'**<**  {GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF["ぜんこくずかんナンバー"] == int(float(dexNumber))-1]["おなまえ"].values[0]:12}'
      )
    ...
    output_log("404 NotFound")
    error404Embed = discord.Embed(
      title="404 NotFound",
      color=0xff0000,
      description=f"The requested {name} was not found on this world",
    )
    error404Embed.set_author(name='Wanaider.com',
      url="https://wiki.ポケモン.com/wiki/ワナイダー",
    )
    error404Embed.set_thumbnail(url=f"{RESOURCES_LINK}art/918.png")
    error404Embed.add_field(name="表記揺れ?",value="別表記を試す")
    error404Embed.add_field(name="不具合?",value="全体チャットで報告")
    
    await interaction.response.send_message(embed=error404Embed)
...
async def slash_chn(interaction: discord.Interaction, name: str):
  ...
  if (pokedata := fetch_pokemon(name)) is not None:
      pokedata = pokedata.fillna(" ")
      dexNumber = pokedata.iloc[0]['ぜんこくずかんナンバー']

      thumbnailUrl = f'{RESOURCES_LINK}art/{dexNumber}.png'

      dexNameZh = str(pokedata.iloc[0]['中国語繁体'])
      dexUrl = f'https://yakkun.com/sv/zukan/?national_no={dexNumber}'

      dexEmbed = discord.Embed(
          title=f'{BOLL_ICON}{dexNameZh}的圖鑑資料{BOLL_ICON}',
          color=0x000000,
          description=f'図鑑番号: {dexNumber}\n中国語繁体名: {dexNameZh}\n拼音: {pinyin_to_text(dexNameZh)}',
          url=dexUrl
      )
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(name="ポケモンの名前",stats="計算するステータス(初期値: すばやさ)",build="努力値と個体値と性格のショートカット",ev="努力値(初期値: 0)",iv="個体値(初期値: 31)",nature="性格補正(初期値: 無補正)",modifier="ランク補正(初期値: 0)",lv="ポケモンのレベル(初期値: 50)")
@discord.app_commands.choices(
...
async def slash_stats(
  ...
  stats: str = "すばやさ",
  build: str = None,
  ev: app_commands.Range[int, 0,252] = 0,
  iv: app_commands.Range[int, 0,31] = 31,
  nature: str = "無補正",
  modifier: app_commands.Range[int, -6,6] = 0,
  lv: app_commands.Range[int, 1,100] = 50
):
  ...
  if (statsPokeData := fetch_pokemon(name)) is not None:
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(a='選択肢1',b='選択肢2',o='選択肢3(任意)',ab='選択肢4(任意)',title='投票のタイトル(任意)',multiple='複数回答の可否')
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(feedback='バグ報告やご意見、ご要望を記述')
async def slash_wish(interaction: discord.Interaction, feedback: str):
  output_log("ご意見受け付けを実行します")
  path="save/feedbacks.csv"
  ...
  userFeedback.to_csv(path, mode='a', index=False,header=not os.path.exists(path))
...
async def slash_spore(interaction: discord.Interaction):
  if interaction.user.guild_permissions.administrator:
    ...
    # csvファイルに書き込み
    memorys.to_csv("save/restMemorychannel.csv", index=False)
    await interaction.response.send_message(f'''```{client.user.name}は
眠ってしまった!```''')
    output_log("botは"+interaction.channel.name+"で眠りにつきました")
    await client.close()
  else:
    await interaction.response.send_message(f"""```{client.user.name}は
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(id='メッセージID',ch='メッセージが存在するチャンネル')
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(month='なん月',data='なん日')
async def slash_calendar(interaction: discord.Interaction,month: app_commands.Range[int, 1,12]=None,data: app_commands.Range[int, 1,31]=None):
  if month is not None and data is not None:
    embed = show_calendar(datetime(datetime.now(ZoneInfo("Asia/Tokyo")).year, month, data, tzinfo=ZoneInfo("Asia/Tokyo")).date())
  else:
    embed = show_calendar()
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(mode='ガイドラインの編集方法を設定します',num='ガイドラインの番号を指定します(未入力ですべてを対象)',channel='編集対象のチャンネルを指定します')
...
async def slash_invite(interaction: discord.Interaction, member: discord.Member, anonymity: bool = False):
  ...
  if not member.bot:
    try:
      ...
      inviteEmbed.set_thumbnail(url=f'{RESOURCES_LINK}icon/RSVP_Mail.png')
      ...
      await member.send(embed=inviteEmbed,view=linkView)
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(goal="参加中の通話の内容や目的")
...
@discord.app_commands.guilds(PDW_SERVER_ID)
@app_commands.describe(user='譲渡したいメンバー')
async def slash_johto(interaction: discord.Interaction, user:  discord.Member):
  ...
  if stageGiveRole in giver.roles:
    if receiver and receiver.voice and receiver.voice.channel.type == discord.ChannelType.stage_voice:
      ...
      output_log(f"{giver.name} が {receiver.name}へホストをジョウトしました")
    else:
      error502Embed=discord.Embed(
        title="502 Bad Gateway",
        color=0xff0000,
        description="nginx"
      )
      error502Embed.set_author(
        name='Strinder.com',
        url="https://wiki.ポケモン.com/wiki/ストリンダー"
      )
      error502Embed.set_thumbnail(url=f'{RESOURCES_LINK}art/849.1.png')
      error502Embed.add_field(name="Receiver is not on stage.",value="ステージチャンネルにいるメンバーを指定してください")
      await interaction.response.send_message(embed=error502Embed,ephemeral=True)
      
  else:
    error403Embed=discord.Embed(
      title="403 Fobbidon",
      color=0xff0000,
      description=f'You don’t have permission to access / {stageGiveRole.name} on this server'
    )
    error403Embed.set_author(
      name='Porygon-Z.com',
      url="https://wiki.ポケモン.com/wiki/ポリゴンZ"
    )
    error403Embed.set_thumbnail(url=f'{RESOURCES_LINK}art/474.png')
    error403Embed.add_field(name="You are not Host",value="ホスト以外はジョウトできません")
    await interaction.response.send_message(embed=error403Embed,ephemeral=True)


def output_log(logStr): #ログプリント用関数
  dt = datetime.now(ZoneInfo("Asia/Tokyo"))
  print(f'[log {dt.hour:02}:{dt.minute:02}:{dt.second:02}]{logStr}')
...
def generate_graph(bss: list[int]) -> str:
  ...
  output_log(f"種族値グラフ生成完了: {location}")
  return location
...
def show_calendar(day: datetime = datetime.now(ZoneInfo("Asia/Tokyo"))) -> discord.Embed:
  ...
  if len(matched_rows) > 0 :
    ...
    if not (eventPokemon := matched_rows.iloc[0]["関連ポケモン"])=="":
      ...
      thumbnailLink = f"{RESOURCES_LINK}art/{eventDexNum}.png"
  else:
...
{BOLL_ICON}`みんなもポケモン ゲットじゃぞ!`'''
  ...
  if not selectedSenryu.iloc[0]['登場ポケモン'] == '':
    ...
    createdEmbed.set_thumbnail(url=f"{RESOURCES_LINK}art/{senryuDexNum}.png")
...
async def on_ready():#bot起動時
  ...
  daily_bonus.start()
  await tree.sync(guild=discord.Object(id=PDW_SERVER_ID))
  
  try : #前回コマンドで終了していたなら
    reminders = pd.read_csv("save/restMemorychannel.csv")
    if not reminders.empty:
      ...
      if channel is not None:
        output_log(f"botは{channel.name}で目を覚ましました\n BAKUSOKU_MODE: {BAKUSOKU_MODE}\n BQ_FILTER_DICT: {BQ_FILTER_DICT}")
        await channel.send(f"```{client.user.name}は\n目を 覚ました!```")
    os.remove("save/restMemorychannel.csv")
  except FileNotFoundError:
    pass
  
  BQ_FILTERED_DF = filterDataframe(BQ_FILTER_DICT).fillna('なし')
  ...
  if(now.hour >= 5):
    ...
    if not timeSignal:
      output_log('本日の時報が取得されませんでした.時報の投稿を試みます')
...
async def on_message(message):
  ...
  #リプライ(reference)に反応
    ...
    if bqFilterWords:
      ...
      BQ_FILTER_DICT.update(make_filter_dict(bqFilterWords))
      BQ_FILTERED_DF = filterDataframe(BQ_FILTER_DICT).fillna('なし')
    ...
    searchedPokeDatas = filterDataframe(make_filter_dict(FilterWords)).fillna('なし')
...
たべのこしで 少し 回復```''')
    ...
    output_log("あいさつを検知しました")
    if any(word in jaconv.kata2hira(message.content) for word in ['おやすみ', 'ねます', 'ねる']):
      hello = "おやす"+random.choice(GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['おなまえ'].str.startswith('ミ')]['おなまえ'].tolist())
      await message.reply("`(つ∀-)"+hello+"ー`")
    else:
      h=datetime.now(ZoneInfo("Asia/Tokyo")).hour
      if h<6:
        hello = "いい夜です"+random.choice(GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['おなまえ'].str.startswith('ネ')]['おなまえ'].tolist())
      elif h<12:
        hello = "おはよ"+random.choice(GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['おなまえ'].str.startswith('ウ')]['おなまえ'].tolist())
      elif h<19:
        hello = "こんにち"+random.choice(GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['おなまえ'].str.startswith('ワ')]['おなまえ'].tolist())
      elif h<20:
        hello = "インテル入って"+random.choice(GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['おなまえ'].str.startswith('ル')]['おなまえ'].tolist())
      else:
        hello = "こんばん"+random.choice(GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['おなまえ'].str.startswith('ワ')]['おなまえ'].tolist())
      await message.reply(f'`{hello}👋😄`')
    output_log("こんにちワニノコ/こんばんワナイダーを実行しました")
...
async def on_voice_state_update(member, before, after):
  ...
  path = "save/busychannel.csv"
    
  if os.path.exists(path):
    call_df = pd.read_csv(path, dtype={"累計参加メンバー": str}).set_index("チャンネルID")
  else:
      ...
      if len(after.channel.members) == 1: #入室時ひとりなら
        ...
        if len(callch.members)>0:
          ...
          if not member.voice or not member.voice.channel == callch: #参加したメンバーがいなくなっていたら
            output_log("参加したメンバーが退出しています")
            return
        else:
          output_log(f'通話は開始されませんでした\n {callch.name}: {member.name}')
          return
      else:
        if not callch.id in call_df.index:
          await asyncio.sleep(5)
          call_df = pd.read_csv(path, dtype={"累計参加メンバー": str}).set_index("チャンネルID") #更新する
    ...
    if after.channel.type == discord.ChannelType.stage_voice:
      ...
      if len(after.channel.members) == 1:
        if stageGiveRole:
          ...
          output_log(f'{member.name}がステージのホストになりました')
        else:
          output_log("ホストロールが存在しませんでした")
      else:
        if not callch.id in call_df.index:
          call_df = pd.read_csv(path, dtype={"累計参加メンバー": str}).set_index("チャンネルID") #更新する
    ...
    if before.channel.type == discord.ChannelType.stage_voice:
      ...
      if (stageGiveRole := discord.utils.get(member.guild.roles,name='🔈エレキトリカル☆ストリーマー')) in member.roles:
        ...
        if len(speakers)==0:
          ...
          output_log(f'{member.name}が抜けたことで{nextSpeaker.name}がステージのホストになりました')
...
async def on_button_click(interaction:discord.Interaction):
    ...
    if custom_id == "authButton": #メンバー認証ボタン モーダルを送信する
      ...
      if report(interaction.user.id,"クジびきけん",0) == 0:
        ...
        lotoEmbed.set_thumbnail(url=f'{RESOURCES_LINK}icon/{prize}.png')
        lotoEmbed.add_field(
...
class CallPost: #await CallPost(*,discord.channel,channelID).start(member,time) /.stop(member,time) /.title(title)
  def __init__(self,channel,sendChannelId: int = None):
    ...
    self.path="save/busychannel.csv"
  ...
  async def start(self,member,time: datetime = datetime.now(ZoneInfo("Asia/Tokyo"))):
    ...
    output_log(f'{self.chType}が開始されました\n {self.channel.name}: {member.name}')
  ...
  async def __load(self):
    if self.channel.id in self.call_df.index:
      try:
        ...
        output_log("ERROR: 指定のメッセージが見つかりませんでした")
        return False
    else:
      output_log("ERROR: 指定チャンネルの通話記録がありません")
      return False
    return True
      ...
      if message.author == client.user and message.embeds:
        for embed in message.embeds:
            if embed.footer and embed.footer.text == f"guidelines: {num}":
              ...
              output_log(f'{num}番目のガイドラインが更新されました')
              return
...
ガイドラインについての質問・要望は<#1069197769434218558>、<#1067408772298969099>、工学院ポケモンだいすきクラブ フォーム,編集メンバーのDMなどをご利用ください。
      ...
      case x:
        output_log(f"{x}は有効なガイドラインナンバーではありません")
...
class quiz():
  ...
  async def try_response(self,response):
    ...
    if isinstance(response, discord.Message):
      ...
      self.ansText = format_text(response.content)
  ...
  async def __judge(self):
    ...
    if self.quizName in ["bq","etojq","ctojq"]:
      if (repPokeData := fetch_pokemon(self.ansText)) is not None:
    ...
    elif judge == "誤答" and self.quizName == "jtoeq" and len((poke := GROBAL_BRELOOM_DF[GROBAL_BRELOOM_DF['英語名'].str.lower() == fixAns])) > 0:
      if isinstance(self.rm, discord.Message):
    ...
    if judge is not None:
      report(self.opener.id, f'{self.quizName}{judge}', 1) #回答記録のレポート
  ...
  async def __disclose(self,tf,answered=None):
    ...
    if self.quizName == "bq":
      ...
      self.quizEmbed.description = f'{bss_to_text(self.ansZero)}\n'
      ...
      if self.quizName == "etojq":
        ...
        self.quizEmbed.description += f'\n拼音: {pinyin_to_text(self.examText)}'
  ...
  def __answers(self):
    ...
    if self.quizName == "bq":
      ...
      aDatas = fetch_pokemon(self.examText)
      aData = aDatas.iloc[0]
  ...
  def __shotgun(self,filter_dict):
    ...
    filteredPokeData = filterDataframe(filter_dict)#.fillna('なし')
